<div align="center">
<img src="banner%20standart%20github.png">

<h1>Network City Helper</h1>

<img alt="Static Badge" src="https://img.shields.io/badge/tag-v1.0.0-blue?style=flat&label=tag&labelColor=gray&color=1284C4">

<img alt="Static Badge" src="https://img.shields.io/badge/%40bot-Network%20City%20Helper-blue?style=flat&label=%40bot&labelColor=gray&color=12C427">

</div>
⠀

> [!CAUTION]
> Бот не имеет отношения к «ИрТеху»
⠀
⠀

## Description

**Network City Helper** - асинхронный телеграм бот для помощи ученикам. Бот может показывать итоговые оценки, уведомлять о просроченных заданиях, отправлять расписание, д/з на завтра и многое другое.

## Credits
[netschoolapi](https://github.com/nm17/netschoolapi) - асинхронный клиент для «Сетевого города»

## Author of Network City Helper
**© Алексеев Роман**
