...
@timetable_load_admin_command.message(Command("timetable_load", prefix='/'))
async def command_timetable_load_handler(message: Message) -> None:
    ...
    if not member_check or not role_check or not auth_check or not privacy_check:
        ...
        if message.photo:
            await bot.download(message.photo[-1], destination=f"source/data/image/расписание.png")

            await message.answer(text=f"<b>Расписание загружено!</b>",
                                 parse_mode=ParseMode.HTML)
        else:
            await message.answer(text="Прикрепите фотографию к команде /timetable_load")
