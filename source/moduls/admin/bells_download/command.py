...
@bells_load_admin_command.message(Command("bells_load", prefix='/'))
async def command_bells_load_handler(message: Message) -> None:
    ...
    if not member_check or not role_check or not auth_check or not privacy_check:
        ...
        if message.photo:
            await bot.download(message.photo[-1], destination=f"source/data/image/расписание_звонков.png")

            await message.answer(text=f"<b>Расписание звонков загружено!</b>",
                                 parse_mode=ParseMode.HTML)
        else:
            await message.answer(text="Прикрепите фотографию к команде /bells_load")
